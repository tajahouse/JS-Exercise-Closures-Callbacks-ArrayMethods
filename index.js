...
function firstNamesAllCaps(runners) {  
  ...
  const inCaps = runners.map(function(name){
...
function getRunnersByTShirtSize(runners, size) {
 let runnerSize = runners.filter(runner => runner.shirt_size === size);
 return runnerSize;
}
...
function tallyUpDonations(runners) {
  const donations = runners.map(amount => amount.donation);
 let total = donations.reduce(function(acc, items){
   return acc + items;
 }, 0);
 return total;
}
